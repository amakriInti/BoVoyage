...
namespace BoVoyage.Donnees
{
    public class Repository
    {
        ...
        public object GetVoyageFormulaire(string continent, string pays, string region)
        {
            ...
            //return query.ToList();
            if (continent == "Fournisseur")
                if (pays == "null" || pays == null) return query.Select(c => c.Fournisseur).ToList().Distinct();
                else return query.Where(c => c.Fournisseur == pays).ToList();

            else if (query.Any(c => c.Region == region))
        ...
        public object DBBanque()
        {
            try
            {
                var query = Context.Banques.ToList();
        ...
        //public object Paiement(Guid? idclient)
        //{
        //    try
        //    {

        //        var query = Context.Banques.Where(b => b.Client == idclient).FirstOrDefault();
        //        return query.ToList();
